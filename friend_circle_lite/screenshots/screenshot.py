...
def _img_auth_code() -> str:
    return os.getenv("IMG_AUTH_CODE", "")


def _img_upload_folder() -> str:
...
def resolve_driver_path() -> str:
    ...
    平台适配：设置 CHROMEDRIVER_PATH 环境变量可直接使用系统 chromedriver
    （如 Debian 的 chromium-driver 包），跳过 webdriver_manager 联网下载，

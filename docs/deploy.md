...
- **友链自助修改（两版通用）**：已交换友链后想改信息，开一个标题以 `[友链修改]` 开头的议题（GitHub 用「友链修改」模板，CNB 直接在议题正文首行写 `[友链修改]`）。填写规则与申请一致，但**只有「网站链接」必填**（用于定位要改的条目），其余字段留空表示不改、填了则覆盖原值；`标签` 字段同样留空不改、填写则覆盖。提交后同样会重新核验可访问性 + 反链，通过即写回 `friends.json` 并联动巡检；若按「网站链接」匹配不到已有友链，会提示改用 `[友链申请]` 从头添加。`remote` 模式下修改通道同样整体关闭。
